...
# --- GLOBAL COLOR MAPPING (Fixed for Likert Scale) ---
# Defining a semantic color scale: Red (Poor) -> Yellow (Fair/Average) -> Green (Good)
sleep_color_map = {
    'Very poor': '#a50026',  # Dark Red
    'Poor':      '#d73027',  # Red
    'Fair':      '#fdae61',  # Orange-Yellow
    'Average':   '#fdae61',  # Handle alternative label
    'Good':      '#66bd63',  # Green
    'Very good': '#1a9850',  # Dark Green
    '(?)':       '#e0e0e0'   # Light Grey for unknowns
}

# Define the logical order for the legend
sleep_quality_order = ['Very poor', 'Poor', 'Fair', 'Average', 'Good', 'Very good', '(?畈)']
...
def load_data(file_path):
    ...
    try:
        ...
        # --- Data Preprocessing for Tab 3: Risk Definition ---
...
# Load data - Using the filename you provided
...
with tab1:
    ...
    if all(col in filtered_df.columns for col in ['age_group', 'sex_label', 'sleep_quality_cat']):
        ...
        fig_sunburst = px.sunburst(
            ...
            color_discrete_map=sleep_color_map, # <--- FIXED COLOR
        ...
        fig_sunburst.update_traces(
            textinfo="label+text",
            texttemplate="<b>%{label}</b><br>%{percentRoot:.1%}", # Simplified percentage display
            hovertemplate=(
                "<b>%{label}</b><br>"
                "Count: %{value}<br>"
                "Share of total: %{percentRoot:.2%}<extra></extra>"
            )
        )
    ...
    with col2_filter: 
        if 'employment_status_main' in filtered_df.columns:
            ...
            selected_employment = st.multiselect("Select Employment Status:", options=employment_options, default=employment_options[0:3] if len(employment_options)>3 else employment_options)
    ...
    with col2_chart:
        ...
        if all(col in filtered_df.columns for col in required_cols):
            ...
            if custom_analysis_df.empty: 
                st.warning("No data points found for selection.")
            else:
                ...
                # Jitter Y-axis slightly for visual separation
                ...
                fig_stress_scatter = px.scatter(
                    df_grouped, 
                    x='eq5d_anxiety_depression', 
                    y='Y_Offset', 
                    size='Count', 
                    color='income_group', 
                    symbol='employment_status_main', 
                    hover_data={'Count': True, 'Y_Offset': False, 'eq5d_anxiety_depression': False, 'employment_status_main': True, 'income_group': True}, 
                    title='Distribution of Anxiety Levels (Bubble Size = Count)', 
                    labels={'eq5d_anxiety_depression': 'Anxiety Level (1=Best, 3=Worst)', 'Y_Offset': ' ', 'employment_status_main': 'Employment Status', 'income_group': 'Income Group'}, 
                    category_orders={"eq5d_anxiety_depression": [1, 2, 3]}
                )
                ...
                fig_stress_scatter.update_layout(
                    yaxis={'visible': False, 'showticklabels': False, 'range': [df_grouped['Y_Offset'].min() - 0.1, df_grouped['Y_Offset'].max() + 0.1]}, 
                    xaxis={'tickvals': [1, 2, 3], 'ticktext': ['1 (Best)', '2', '3 (Worst)'], 'range': [0.5, 3.5], 'title': 'Anxiety Level'}, 
                    height=500
                )
                st.plotly_chart(fig_stress_scatter, use_container_width=True)
        else: 
            st.warning("The dataset is missing columns required for the Bubble Chart.")
    st.markdown("---")
    ...
    if all(col in filtered_df.columns for col in ['bmi_group', 'sleep_quality_cat']):
        ...
        # Calculate Percentage within BMI group
        ...
        # --- FIX 1: Sort the Dataframe to prevent "Zig-Zag" lines ---
        bmi_order = ['Underweight', 'Normal', 'Overweight', 'Obese', '(?)']
        # Convert BMI to categorical type with order, then sort
        df_bmi_sleep['bmi_group'] = pd.Categorical(df_bmi_sleep['bmi_group'], categories=bmi_order, ordered=True)
        df_bmi_sleep = df_bmi_sleep.sort_values('bmi_group')
        
        fig_bmi_sleep = px.line(
            df_bmi_sleep, 
            x='bmi_group', 
            y='Percentage', 
            color='sleep_quality_cat', 
            title='Percentage of Sleep Quality Categories Across BMI Groups', 
            category_orders={'bmi_group': bmi_order, 'sleep_quality_cat': sleep_quality_order},
            labels={'bmi_group': 'BMI Group', 'sleep_quality_cat': 'Sleep Quality Category'}, 
            color_discrete_map=sleep_color_map, # <--- FIXED COLOR
            markers=True
        )
...
with tab2:
    ...
    if all(col in filtered_df.columns for col in required_cols_2_1):
        ...
        fig_sleep_scatter = px.scatter(
            filtered_df, 
            x='sleep_hours', 
            y='mental_health_jitter', 
            color='sleep_quality_cat', 
            hover_data=['age_group', 'sex_label', 'sleep_hours', 'global_mental_health', 'sleep_quality_cat'], 
            title='Sleep Duration vs. Mental Health Rating', 
            labels={'sleep_hours': 'Reported Sleep Duration (Hours)', 'mental_health_jitter': 'Global Mental Health Rating (1=Poor, 5=Best)', 'sleep_quality_cat': 'Sleep Quality'}, 
            color_discrete_map=sleep_color_map, # <--- FIXED COLOR
            category_orders={'sleep_quality_cat': sleep_quality_order},
            trendline="ols" 
        )
        fig_sleep_scatter.update_layout(
            yaxis={'tickvals': [1, 2, 3, 4, 5], 'ticktext': ['1 (Poor)', '2', '3', '4', '5 (Best)'], 'title': 'Global Mental Health Rating', 'range': [0.5, 5.5], 'autorange': False }, 
            height=550
        )
    ...
    if all(col in filtered_df.columns for col in radar_cols_raw + radar_cols_inverted + ['sleep_quality_cat']):
        ...
        categories = ['Physical Health', 'Mental Health', 'Quality of Life', 'Low Fatigue']
        
        fig_radar = go.Figure()
        
        # Ensure we loop through the logical order (Poor -> Good) so legend looks nice
        existing_qualities = [q for q in sleep_quality_order if q in df_radar['sleep_quality_cat'].values]
        
        for quality in existing_qualities:
            row = df_radar[df_radar['sleep_quality_cat'] == quality].iloc[0]
            values = row[radar_cols_raw + radar_cols_inverted].tolist()
            # Close the loop for radar chart
            values += values[:1]
            cat_closed = categories + categories[:1]
            ...
            fig_radar.add_trace(go.Scatterpolar(
                ...
                fill='toself', 
                name=quality, 
                line_color=sleep_color_map.get(quality, '#C0C0C0') # <--- FIXED COLOR
            ))
            
        fig_radar.update_layout(
            title='Average Health Profile by Sleep Quality (Larger Area = Better Health)', 
    ...
    if all(col in filtered_df.columns for col in required_cols_2_3):
        ...
        fig_violin = px.violin(
            ...
            box=True, 
            points='outliers', 
            category_orders={'eq5d_anxiety_depression': anxiety_order, 'sleep_quality_cat': sleep_quality_order}, 
            color_discrete_map=sleep_color_map, # <--- FIXED COLOR
            title='Distribution of Sleep Latency by Anxiety Level'
        )
        fig_violin.update_layout(xaxis_title='Anxiety Level (1=Best, 3=Worst)', yaxis_title='Sleep Latency (Minutes)', height=550)
...
# ====================================================================
# TAB 3: Personalized Profile Explorer
# ====================================================================
...
with tab3:
    ...
    st.markdown("""
        **Instruction:** Select up to three personal factors below. The chart will dynamically update to show the **distribution of Sleep Quality** for those specific combinations.
    """)
    ...
    # Apply Factor 3 as a filter if selected
    ...
    if all(col in df_analysis.columns for col in required_cols_3_1):
        ...
        if df_grouped.empty:
            st.warning(f"No groups found with N >= {min_group_size} for the selected combination.")
        else:
            title_text = f"Sleep Quality Distribution by Profile: {factor_1.replace('_', ' ').title()} | {factor_2.replace('_', ' ').title()}" + f3_label
            ...
            fig_stacked = px.bar(
                ...
                color_discrete_map=sleep_color_map, # <--- FIXED COLOR
                title=title_text,
                labels={'Percentage': 'Proportion of Group (%)', 'Combined_Group': 'Profile Combination'},
            ...
            st.markdown(f"""
            > **How to Read:** The **Red** sections represent the proportion of people with **Poor** sleep. Groups with larger red bars are at higher risk. The **Green** sections represent **Good** sleep.
            """)
